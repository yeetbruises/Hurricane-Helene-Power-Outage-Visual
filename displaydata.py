...
# Load the Shapefile
shapefile_path = './shapefiles/cb_2018_us_county_20m.shp'  # Replace with your shapefile path
...
output_csv_path = './outagedata/outputnew.csv'  # Replace with your output CSV path
output_df = pd.read_csv(output_csv_path)

# Load the county FIPS CSV file
county_fips_path = 'county-fips.csv'  # Replace with your county FIPS CSV path
...
def get_population_by_county(county_name, state):
    # Load the output CSV file
    cdf = 'uscounties.csv'  # Replace with your output CSV path
    cdf = pd.read_csv(cdf)

    # Filter the DataFrame for the given county name
    result = cdf[(cdf['county'].str.lower() == county_name.lower()) & (cdf['state_id'].str.lower() == state.lower())]  # Case-insensitive match
...
def update(frame):
    ...
    for county, data_point in row.items():
        ...
        output_shapefile_path = 'shapefiles/output/added_color.shp'  # Replace with your output path
        gdf.to_file(output_shapefile_path)

    minx, maxx = -100, -60   # Longitude limits
    miny, maxy = 24, 40      # Latitude limits
    ...
    # FRAME += 1
    plt.savefig(f'render/my_plot{frame}.png', dpi=400, bbox_inches='tight')
    b = time.time()
